...
import { FileText, Search, Trash2, Upload, ExternalLink, Filter } from "lucide-react";
...
function TutorResources() {
  ...
  const myResources = resources.filter((r) => r.tutor && r.tutor._id === user?._id);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-slate-100 flex items-center gap-2">
            <FileText className="text-indigo-600 dark:text-indigo-400" size={28} /> Learning Resources
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
            Upload and manage study materials, notes, and assignments for students.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="You haven't uploaded any learning materials yet. Click the button above to get started."
        />
      ) : (
